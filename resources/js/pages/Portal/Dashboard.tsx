            ...
            <div className="space-y-4">
                {enrollments.map((enrollment) => (
                    <div key={enrollment.id} className="rounded-lg border border-[#1F2A24]/10 bg-white p-5 shadow-sm">
                        ...
                        <div className="mt-4">
                            {enrollment.cancelled_at ? (
                                <p className="text-sm text-gray-500">This application was cancelled.</p>
                            ) : enrollment.enrollment_status === 'PENDING' ? (
                                <p className="text-sm text-[#1F2A24]/60">
                                    Your application is under review. We'll notify you by email once it's decided.
                                </p>
                            ) : enrollment.enrollment_status === 'REJECTED' ? (
                                <p className="text-sm text-red-600">
                                    This application was not approved. Contact the registrar's office for details.
                                </p>
                            ) : (
                            ...
                            {!enrollment.cancelled_at && enrollment.enrollment_status === 'PENDING' && (
                                <Button variant="outline" size="sm" className="mt-2 block" onClick={() => cancel(enrollment.id)}>
                                    Cancel Application
                                </Button>
                            )}
                        </div>
